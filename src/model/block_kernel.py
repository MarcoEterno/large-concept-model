...
class GeneralCausalSelfAttention(nn.Module):
    ...
    def forward(self, xt, xc):
        ...
        xc_new = xc.transpose(1, 2).contiguous().view(B, C, Dc)
...
class GeneralBlock(nn.Module):
    ...
    def forward(self, xt, xc):
        attn_xt, attn_xc = self.attn(self.ln_1t(xt), self.ln_1c(xc)) # self.ln_1c(xc) in general
        xt = xt + attn_xt
        xc = xc + attn_xc
        mlp_xt, mlp_xc = self.mlp(self.ln_2t(xt), self.ln_1c(xc)) # self.ln_1c(xc) in general
        xt = xt + mlp_xt
        xc = xc + mlp_xc
        return xt, xc
