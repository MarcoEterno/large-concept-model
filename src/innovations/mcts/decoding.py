...
@timer
def get_llm_probabilities(encoded_text, model:GPT2LMHeadModel, return_type, device=DEVICE):
    """
...
@timer
def old_old_get_n_tokens_inference(inputs, model, n_tokens, input_is_encoded=False, device=DEVICE):
    """
...
def old_get_n_tokens_inference(
        previous_tokens: list[int],
        model,
        tokenizer,
