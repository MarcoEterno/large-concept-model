...
def load_tokens(filename, device='cpu'):
...
class DataLoaderWithConcepts:
    def __init__(self, B, T, process_rank, num_processes, split, master_process, device='cpu'):
        self.B = B
        self.T = T
        self.process_rank = process_rank
        self.num_processes = num_processes
    ...
    def next_batch(self):
        ...
        # calculating the input concepts
        concepts = self.encoder.encode_tokens(x).view(B, T // N_TOKENS_PER_CONCEPT, -1)
