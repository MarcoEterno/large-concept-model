...
from src.model.config import DATA_ROOT_PATH, GPTConfig
...
for step in range(max_steps):
    ...
    if step > 1 and (step % 100 == 0 or last_step) and (not use_compile):
        num_correct_norm = 0
        num_total = 0
    ...
    if ((step > 1 and step % 250 == 0) or last_step) and (not use_compile):
        model.eval()
